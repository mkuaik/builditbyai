# 🎯 خارطة طريق التسجيل في برامج الأفليت (Affiliate Roadmap)

هذا الملف مُصمم لمتابعة عملية التسجيل في أفضل 20 أداة تم اختيارها لمشروع **DoItByAI**.

---

## 🛠️ متطلبات التسجيل الأساسية (قبل البدء)
يفضل تجهيز هذه المعلومات في ملف نصي لتسريع عملية التقديم:
1. **رابط الموقع**: `builditbyai.com` (أو الرابط الحالي للمشروع).
2. **وصف القناة**: "دليل عربي شامل لأدوات الذكاء الاصطناعي يستهدف صناع المحتوى والشركات في الشرق الأوسط".
3. **وسيلة الاستلام**: حساب PayPal (أو Payoneer لبعض الأدوات).

---

## 📈 سجل متابعة التسجيل (20 أداة مختارة)

| الأداة | رابط صفحة التسجيل (Direct Signup) | الحالة | ملاحظات |
| :--- | :--- | :--- | :--- |
| **Beehiiv** | [Register Here](https://www.beehiiv.com/affiliates) | [x] مقبول ✅ | أعلى عمولة (60%) |
| **Framer** | [Register Here](https://www.framer.com/partners/) | [x] قيد المراجعة | عمولة 50% |
| **Notion** | [Register Here](https://www.notion.so/affiliates) | [x] قيد المراجعة | عبر PartnerStack |
| **Copy.ai** | [Register Here](https://www.copy.ai/affiliate-program) | [x] قيد المراجعة | عبر PartnerStack |
| **Pictory** | [Register Here](https://partners.pictory.ai/login) | [x] مقبول ✅ | عمولة 20% مدى الحياة |
| **ElevenLabs** | [Register Here](https://elevenlabs.io/affiliates) | [x] قيد المراجعة | عبر PartnerStack |
| **Leonardo AI** | [Register Here](https://leonardo.ai/affiliate-program/) | [x] قيد المراجعة | عبر Impact.com |
| **Koala AI** | [Register Here](https://koala.sh/affiliate) | [x] قيد المراجعة | عبر البريد الإلكتروني ✅ |
| **Semrush** | [Register Here](https://www.semrush.com/affiliate/) | [x] قيد المراجعة | عبر Impact.com |
| **InVideo AI** | [Register Here](https://invideo.io/affiliate-program/) | [x] قيد المراجعة | عبر Impact.com |
| **Jasper** | [Register Here](https://www.jasper.ai/affiliate) | [ ] لم يبدأ | عمولة 30% |
| **Writesonic** | [Register Here](https://writesonic.com/affiliate) | [x] قيد المراجعة | دعم ممتاز للعربية ✅ |
| **SurferSEO** | [Register Here](https://surferseo.com/affiliate/) | [x] قيد المراجعة | عبر PartnerStack |
| **Synthesia** | [Register Here](https://www.synthesia.io/affiliate-program) | [ ] لم يبدأ | اشتراكات غالية |
| **HeyGen** | [Register Here](https://www.heygen.com/affiliate) | [ ] لم يبدأ | ترند الترجمة |
| **Taskade** | [Register Here](https://www.taskade.com/affiliates) | [x] مقبول ✅ | عمولة لمدى الحياة |
| **Topaz Labs** | [Register Here](https://topazlabs.com/affiliate) | [x] قيد المراجعة ⏳ | جودة صور وفيديو احترافية |
| **Murf AI** | [Register Here](https://murf.ai/affiliate) | [ ] لم يبدأ | عقود شركات |
| **Chatbase** | [Register Here](https://www.chatbase.co/affiliate) | [ ] لم يبدأ | حلول بزنس |
| **Canva** | [Register Here](https://www.canva.com/affiliates/) | [ ] لم يبدأ | سهلة الإقناع |
| **Looka** | [Register Here](https://looka.com/affiliates/) | [ ] لم يبدأ | لرواد الأعمال |

---

## ⚡ أدوات "التسجيل السريع" (Direct & Fast Signup)
هذه الأدوات لا تتطلب المرور بـ (Marketplace) معقد، وتستخدم غالباً قنوات تسجيل مباشرة أو منصات بسيطة مثل Rewardful يتم القبول فيها خلال دقائق.

| الأداة | رابط التسجيل المباشر | العمولة | ملاحظة |
| :--- | :--- | :--- | :--- |
| **Writesonic** | [Sign Up](https://writesonic.com/affiliate) | 30% متكررة | قوية جداً للعربية |
| **Rytr** | [Sign Up](https://rytr.me/affiliate) | 30% مدى الحياة | بونص 15$ عند البدء |
| **Submagic** | [Sign Up](https://submagic.co/affiliate) | 30% مدى الحياة | ترند صناعة المحتوى |
| **HeyGen** | [Sign Up](https://heygen.com/affiliate) | 20% متكررة | أفضل أفاتار حالياً |
| **Synthesia** | [Sign Up](https://synthesia.io/affiliate-program) | 20% متكررة | رائدة الفيديو الرقمي |
| **Frase** | [Sign Up](https://frase.io/affiliate-program) | 30% متكررة | ممتازة للسيو |
| **Lovo AI** | [Sign Up](https://lovo.ai/affiliate) | 20% متكررة | أصوات احترافية |
| **AdCreative** | [Sign Up](https://adcreative.ai/affiliate) | 25-50% | عمولات مجزية |
| **Mixo.io** | [Sign Up](https://mixo.io/affiliate) | 20% متكررة | بناء مواقع بدقيقة |
| **Chatbase** | [Sign Up](https://chatbase.co/affiliate) | 20% متكررة | بوتات ذكاء اصطناعي |

---

## 💡 نصيحة المنسق (نور):
1. ابدأ بقسم "التسجيل السريع" إذا كنت تشعر بالتعب من المنصات الكبرى، فالقبول فيها أسرع بكثير.
2. بمجرد حصولك على "رابط الأفليت الخاص بك"، ضعه في خانة الملاحظات هنا، وسأقوم أنا بنقله للكود البرمجي للموقع فوراً.
